...
local function apply_patch(filepath, pattern, replacement, label)
    ...
    if count == 0 then
        log.tmp_warn("Patch [" .. label .. "] failed on " .. filepath .. " (Apotheosis updated?)")
        return false
    end
    ModTextFileSetContent(filepath, result)
    log.tmp_info("Patch [" .. label .. "] applied: " .. filepath)
    return true
end
...
local function apply_patch_convert(new_inputs, new_outputs)
    ...
    if c1 == 0 or c2 == 0 then
        log.tmp_warn("Patch [perk_creation_convert] failed on " .. filepath .. " (Apotheosis updated?)")
        return false
    end
    ModTextFileSetContent(filepath, result2)
    log.tmp_info("Patch [perk_creation_convert] applied: " .. filepath)
    return true
end

--- Adds the blankStone_perk_reforge tag to every reforge perk entity.
local function apply_reforge_tags(reforge_perks)
    for _, path in pairs(reforge_perks) do
        for xml in nxml.edit_file(path .. ".xml") do
            local root = xml
            if root.attr.tags then
                if not root.attr.tags:find("blankStone_perk_reforge") then
                    ...
                    log.tmp_warn("Tag blankStone_perk_reforge already present: " .. path)
                end
            else
