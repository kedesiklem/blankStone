...
dofile_once( "mods/blankStone/files/scripts/storage_stone/utils/noita_utils.lua" )
...
function universal_bag_pickup(entity_who_kicked, active_item)
    ...
    -- Pickup spells
    if ModSettingGet("blankStone.allow_spells") then
    ...
    -- Pickup wands
    if ModSettingGet("blankStone.allow_wands") then
    ...
    -- Pickup potions
    if ModSettingGet("blankStone.allow_potions") then
    ...
    -- Pickup items
    if ModSettingGet("blankStone.allow_items") then
        local entities = EntityGetInRadius(pos_x, pos_y, pickup_distance)
...
function is_same_entity_type(entity_one, entity_two)
    ...
    if is_storageStone(entity_one) then
        entity_one_type = 0
    ...
    if is_storageStone(entity_two) then
        entity_two_type = 0
...
function is_bag_inventory(inventory)
    if inventory then
        ...
        if inv_parent and is_storageStone(inv_parent) then
            return true
        end
    end
    return false
...
--- @return boolean
function is_storageStone(entity_id)
    if EntityGetName(entity_id) == nil then
        return false
    end
    return string.find(EntityGetName(entity_id), "storageStone") ~= nil
end
...
function is_allowed_in_universal_bag(entity_id)
    ...
    if ModSettingGet("blankStone.allow_bags_inception") then
        is_a_bag = is_storageStone(entity_id)
    end
...
function is_bag_one_smaller_than_bag_two(bag_one, bag_two)
    if not is_storageStone(bag_one) or not is_storageStone(bag_two) then
        return true
    end
...
function is_allowed_in_bag(item_id, bag_id)
    if is_storageStone(item_id) and is_in_bag_tree(item_id, bag_id) then
        return false
    end
...
function is_allowed_in_inventory(item_id, inv_id, position)
    if is_storageStone(inv_id) then
...
function drop_all_inventory(bag, orderly, sort_by_time, sorting_order)
    ...
    if orderly then
        local spacing = tonumber(ModSettingGet("blankStone.drop_orderly_distance"))
        if not spacing then
            spacing = 10
        else
    ...
    for _, entity in ipairs(entities) do
        if EntityGetParent(entity) == 0 then
            if is_bag_not_full(active_item, get_bag_inventory_size(active_item)) then
                if not ModSettingGet("blankStone.allow_holy_mountain_spell_stealing") then
...
function add_wands_to_inventory(active_item, inventory, player_id, entities)
    for _, entity in ipairs(entities) do
        ...
        if not ModSettingGet("blankStone.allow_tower_wand_stealing") then
            if EntityHasTag(entity, "wand_good") then
                can_be_picked_up = false
            end
        end
        if not ModSettingGet("blankStone.allow_holy_mountain_wand_stealing") then
...
function add_items_to_inventory(active_item, inventory, player_id, entities)
    for _, entity in ipairs(entities) do
        local can_be_picked_up = true
        if not ModSettingGet("blankStone.allow_sampo_stealing") then
            if EntityHasTag(entity, "this_is_sampo") then
                can_be_picked_up = false
            end
        end
        if not is_storageStone(entity) and entity ~= active_item and item_pickup_is_pickable_in_inventory(entity) then
            if EntityGetParent(entity) == 0 and can_be_picked_up then
                if is_bag_not_full(active_item, get_bag_inventory_size(active_item)) then
                    add_entity_to_inventory_bag(get_smallest_position_avalaible(active_item), inventory, entity)
                end
...
function get_bag_inventory_size(entity_id)
    if entity_id then
        ...
        if bag_name then
            local size = tonumber(ModSettingGet("blankStone.".. bag_name .. "_size"))
            if not size then
                size = 0
            else
...
function get_sprite_file( entity_id )
    local sprite = "mods/blankStone/files/ui_gfx/inventory/unidentified_item.png"
    ...
    if sprite == "mods/blankStone/files/ui_gfx/inventory/unidentified_item.png" then
        local item_component = EntityGetComponentIncludingDisabled(entity_id, "SpriteComponent")
        if item_component then
            local item_sprite = ComponentGetValue2(item_component[1], "image_file")
            if item_sprite and item_sprite ~= "" then
