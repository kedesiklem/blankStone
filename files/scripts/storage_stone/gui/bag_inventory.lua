...
dofile_once( "mods/blankStone/files/scripts/storage_stone/utils/spawn.lua" )
dofile_once( "mods/blankStone/files/scripts/storage_stone/utils/utils.lua" )
dofile_once( "mods/blankStone/files/scripts/storage_stone/utils/inventory.lua" )
dofile_once( "mods/blankStone/files/scripts/storage_stone/utils/wand_and_spells.lua" )
...
-- MOD SETTINGS
local show_bags_without_inventory_open = ModSettingGet("blankStone.show_bags_without_inventory_open")
local only_show_bag_button_when_held = ModSettingGet("blankStone.only_show_bag_button_when_held")
local bag_ui_red = tonumber(ModSettingGet("blankStone.bag_image_red"))/255
local bag_ui_green = tonumber(ModSettingGet("blankStone.bag_image_green"))/255
local bag_ui_blue = tonumber(ModSettingGet("blankStone.bag_image_blue"))/255
local bag_ui_alpha = tonumber(ModSettingGet("blankStone.bag_image_alpha"))/255
local dragging_allowed = get_mod_setting_boolean("blankStone.dragging_allowed", true)
local item_per_line = get_mod_setting_number("blankStone.bag_slots_inventory_wrap", 8)

-- BAG AND TOOLTIP TOGGLE
local bag_open = true
local alchemy_gui_open = false
local keep_tooltip_open = get_mod_setting_boolean("blankStone.keep_tooltip_open", false)
local dropdown_style = get_mod_setting_boolean("blankStone.dropdown_style", false)

-- SORTING FLAG AND OPTION
local sort_type_change_flag = false
local sort_by_time_setting = get_mod_setting_boolean("blankStone.sorting_type", false)
local sort_order_change_flag = false
local sorting_order = get_mod_setting_boolean("blankStone.sorting_order", false)
-- OPTION CHANGE FLAG
local option_changed = false

local drop_no_order = "mods/blankStone/files/ui_gfx/inventory/bag_gui_button_drop.png"
local drop_orderly = "mods/blankStone/files/ui_gfx/inventory/bag_gui_button_drop_orderly.png"
...
local function update_settings()
    show_bags_without_inventory_open = ModSettingGet("blankStone.show_bags_without_inventory_open")
    only_show_bag_button_when_held = ModSettingGet("blankStone.only_show_bag_button_when_held")
    bag_ui_red = tonumber(ModSettingGet("blankStone.bag_image_red"))/255
    bag_ui_green = tonumber(ModSettingGet("blankStone.bag_image_green"))/255
    bag_ui_blue = tonumber(ModSettingGet("blankStone.bag_image_blue"))/255
    bag_ui_alpha = tonumber(ModSettingGet("blankStone.bag_image_alpha"))/255
    dragging_allowed = get_mod_setting_boolean("blankStone.dragging_allowed", true)
    item_per_line = get_mod_setting_number("blankStone.bag_slots_inventory_wrap", 8)
    sort_by_time_setting = get_mod_setting_boolean("blankStone.sorting_type", false)
    sorting_order = get_mod_setting_boolean("blankStone.sorting_order", false)
    keep_tooltip_open = get_mod_setting_boolean("blankStone.keep_tooltip_open", false)
end
...
local function draw_bag_layers_and_inventory(active_item, pos_x, pos_y, level)
    ...
    for bag_level, bag in pairs(inventory_bag_table[active_item_bag]) do
        ...
        if is_potion_bag(bag) or (ModSettingGet("blankStone.universal_bag_alchemy_table") and is_universal_bag(bag)) then
...
local function handle_option_changes()
    if sort_order_change_flag then
        ...
        ModSettingSetNextValue("blankStone.sorting_order", sorting_order, false)
    end
    if sort_type_change_flag then
        sort_type_change_flag = false
        sort_by_time_setting = not sort_by_time_setting
        ModSettingSetNextValue("blankStone.sorting_type", sort_by_time_setting, false)
    end
end
...
function bags_of_many_bag_gui(pos_x, pos_y)
    ...
    local active_item_is_bag = is_storageStone(active_item)
...
function button_test(pos_x, pos_y)
    ...
    if player then
        ...
        local lc, rc = GuiImageButton(gui, bags_of_many_new_id(), pos_x, pos_y, "", "mods/blankStone/files/ui_gfx/inventory/bag_gui_button_options.png")
        if lc or rc then
...
function multi_layer_bag_image_v2(bag, bag_display_x, bag_display_y, pos_z, level)
    ...
    if bag_hovered_sprite then
        if bag_pickup_override_local == bag then
            ...
            GuiImage(gui, bags_of_many_new_id(), bag_display_x, bag_display_y, "mods/blankStone/files/ui_gfx/inventory/bag_pickup_override_inventory.png", 1, 1)
        end
        -- Background for bag inception display
        local gui_button_image = "mods/blankStone/files/ui_gfx/inventory/bag_gui_button.png"
...
function draw_inventory_v2(active_item, pos_x, pos_y, pos_z, entity, level)
    ...
    if is_storageStone(entity) and active_item then
...
function draw_inventory_v2_invisible(storage_size, positions, bag, level)
    ...
    for i = 1, storage_size do
        ...
        if not(dragged_item_table.item and dragged_item_table.bag == bag and dragged_item_table.level == level and dragged_item_table.position == i) and active_item_bag then
            ...
            GuiZSetForNextWidget(gui, 1)
            GuiImageButton(gui, bags_of_many_new_id(), item_pos_x, item_pos_y, "", "mods/blankStone/files/ui_gfx/inventory/box/invisible20x20.png")
            ...
            if hovered_inv then
                ...
                if dragged_item_table.item and is_allowed_in_bag(dragged_item_table.item, hovered_item_table.bag) and not is_in_bag_tree(dragged_item_table.item, hovered_item_table.bag) and not is_in_bag_tree(hovered_item_table.item, dragged_item_table.bag) then
                    ...
                    GuiImage(gui, bags_of_many_new_id(), item_pos_x, item_pos_y, "mods/blankStone/files/ui_gfx/full_inventory_box_highlight.png", 1, 1)
                end
            end
        end
    end
                ...
                if item_pos_x and item_pos_y then
                    ...
                    if right_click_table.position == item_position and right_click_table.level == level and right_click_table.bag == bag then
                        ...
                        if is_potion(item) then
                            ...
                            if not dropdown_style and is_storageStone(item) and active_item_bag then
                    ...
                    if hovered_item_table.position == item_position and hovered_item_table.level == level and hovered_item_table.bag == bag and not left_click_table.item then
                        ...
                        if not is_storageStone(item) and not dragged_item_table.item then
                            draw_tooltip(item, item_pos_x, item_pos_y, level)
                        end

                        if dropdown_style and is_storageStone(item) and inventory_bag_table[active_item_bag][level+1] ~= item then
                            remove_draw_list_under_level(inventory_bag_table[active_item_bag], level)
                            inventory_bag_table[active_item_bag][level+1] = item
                        end
                    end
                    ...
                    if sprite_path and not(dragged_item_table.item == item) then
                        ...
                        if inventory_bag_table[active_item_bag][level+1] == item then
                            GuiZSetForNextWidget(gui, pos_z + 2)
                            GuiImage(gui, bags_of_many_new_id(), item_pos_x - pad_x, item_pos_y - pad_y + hover_animation, "mods/blankStone/files/ui_gfx/inventory/bag_open_inventory.png", 1, 1)
                        end
                        -- PICKUP OVERRIDE ANIMATION
                        if bag_pickup_override_local == item then
                            GuiZSetForNextWidget(gui, pos_z + 8)
                            GuiImage(gui, bags_of_many_new_id(), item_pos_x - pad_x, item_pos_y - pad_y + hover_animation, "mods/blankStone/files/ui_gfx/inventory/bag_pickup_override_inventory.png", 1, 1)
                        end
                        if item == left_spot_alchemy.item then
                            GuiZSetForNextWidget(gui, pos_z + 8)
                            GuiImage(gui, bags_of_many_new_id(), item_pos_x - pad_x, item_pos_y - pad_y + hover_animation, "mods/blankStone/files/ui_gfx/potion_mixing/alchemy_spot_left.png", 1, 1)
                        elseif item == combined_spot_alchemy.item then
                            GuiZSetForNextWidget(gui, pos_z + 8)
                            GuiImage(gui, bags_of_many_new_id(), item_pos_x - pad_x, item_pos_y - pad_y + hover_animation, "mods/blankStone/files/ui_gfx/potion_mixing/alchemy_spot_combined.png", 1, 1)
                        elseif item == right_spot_alchemy.item then
                            GuiZSetForNextWidget(gui, pos_z + 8)
                            GuiImage(gui, bags_of_many_new_id(), item_pos_x - pad_x, item_pos_y - pad_y + hover_animation, "mods/blankStone/files/ui_gfx/potion_mixing/alchemy_spot_right.png", 1, 1)
                        end
                    end
                end
            end
...
function draw_inventory_dragged_item_v2(pos_z)
    ...
    if item and dragged_invis_gui_id and dragged_item_gui_id then
        ...
        GuiOptionsAddForNextWidget(gui, GUI_OPTION.DrawNoHoverAnimation)
        GuiZSetForNextWidget(gui, 1)
        GuiImageButton(gui, dragged_invis_gui_id, 100, 100, "", "mods/blankStone/files/ui_gfx/inventory/box/invisible20x20.png")
        local clicked_inv, right_clicked_inv, hovered_inv, _, _, _, _, draw_x, draw_y = GuiGetPreviousWidgetInfo(gui)
...
function detect_vanilla_wand_inventory_mouse_inputV2(gui, pos_x, pos_y, pos_z, item, inv_spot)
    ...
    for i = 1, wand_capacity do
        ...
        GuiOptionsAddForNextWidget(gui, GUI_OPTION.DrawNoHoverAnimation)
        GuiZSetForNextWidget(gui, pos_z)
        GuiImageButton(gui, bags_of_many_new_id(), pos_x, pos_y, "", "mods/blankStone/files/ui_gfx/inventory/box/invisible20x20.png")
        ...
        if hover then
            ...
            if dragged_item_table.item and not vanilla_inventory_table.quick.widget_item then
                ...
                if should_display_hover_anim then
                    GuiZSetForNextWidget(gui, 1)
                    GuiImage(gui, bags_of_many_new_id(), pos_x - 1, pos_y - 1, "mods/blankStone/files/ui_gfx/full_inventory_box_highlight.png", 1, 1.1)
                end
            end
        end
...
function draw_left_bracket(gui, id, pos_x, pos_y, pos_z)
    ...
    GuiZSetForNextWidget(gui, pos_z)
    GuiImage(gui, id, pos_x-5, pos_y - 4, "mods/blankStone/files/ui_gfx/inventory/background_left.png", bag_ui_alpha, 1)
end
...
function draw_middle(gui, id, pos_x, pos_y, pos_z)
    GuiColorSetForNextWidget(gui, bag_ui_red, bag_ui_green, bag_ui_blue, 1)
    GuiZSetForNextWidget(gui, pos_z)
    GuiImage(gui, id, pos_x, pos_y - 4, "mods/blankStone/files/ui_gfx/inventory/background_middle.png", bag_ui_alpha, 1)
end
...
function draw_right_bracket(gui, id, pos_x, pos_y, pos_z)
    GuiColorSetForNextWidget(gui, bag_ui_red, bag_ui_green, bag_ui_blue, 1)
    GuiZSetForNextWidget(gui, pos_z)
    GuiImage(gui, id, pos_x+20, pos_y - 4, "mods/blankStone/files/ui_gfx/inventory/background_right.png", bag_ui_alpha, 1)
end
...
function draw_inventory_button(pos_x, pos_y, active_item)
    ...
    if not ModSettingGet("blankStone.locked") then
        if not bag_pickup_override_local or bag_pickup_override_local == 0 then
            ...
            GuiImage(gui, bags_of_many_new_id(), pos_x, pos_y, "mods/blankStone/files/ui_gfx/inventory/bag_pickup_override_inventory.png", 1, 1)
        end
        GuiOptionsAddForNextWidget(gui, GUI_OPTION.NoPositionTween)
        GuiOptionsAddForNextWidget(gui, GUI_OPTION.ClickCancelsDoubleClick)
        GuiOptionsAddForNextWidget(gui, GUI_OPTION.NoSound)
        ...
        GuiOptionsAddForNextWidget(gui, GUI_OPTION.DrawNoHoverAnimation)
        GuiZSetForNextWidget(gui, 1)
        clicked, right_clicked = GuiImageButton(gui, bags_of_many_new_id(), pos_x, pos_y, "", "mods/blankStone/files/ui_gfx/inventory/box/invisible20x20.png")
    ...
    -- Bag button
    local bag_sprite = "mods/blankStone/files/ui_gfx/inventory/drag_icon.png"
    if is_storageStone(active_item) then
...
function draw_inventory_sorting_option(pos_x, pos_y, pos_z)
    GuiZSetForNextWidget(gui, pos_z)
    GuiColorSetForNextWidget(gui, bag_ui_red, bag_ui_green, bag_ui_blue, bag_ui_alpha)
    local sorting_option_sprite =  "mods/blankStone/files/ui_gfx/inventory/bag_gui_button_sorting_by_position.png"
    local sorting_option_tooltip = "$bag_button_sorting_by_position_tooltip"
    if sort_by_time_setting then
        sorting_option_sprite =  "mods/blankStone/files/ui_gfx/inventory/bag_gui_button_sorting_by_time.png"
...
function draw_inventory_sorting_direction(pos_x, pos_y, pos_z)
    GuiZSetForNextWidget(gui, pos_z)
    GuiColorSetForNextWidget(gui, bag_ui_red, bag_ui_green, bag_ui_blue, bag_ui_alpha)
    local sorting_direction_sprite =  "mods/blankStone/files/ui_gfx/inventory/bag_gui_button_sort_desc.png"
    local sorting_direction_tooltip = "$bag_button_tooltip_desc_sort"
    if sorting_order then
        sorting_direction_sprite =  "mods/blankStone/files/ui_gfx/inventory/bag_gui_button_sort_asc.png"
...
function draw_inventory_drop_button(bag, pos_x, pos_y, pos_z, level)
    ...
    if l_clk and type(sort_by_time_setting) == "boolean" and type(sorting_order) == "boolean" then
        ...
        remove_draw_list_under_level(inventory_bag_table[active_item_bag], level)
    elseif r_clk then
        if dropping_button_sprite == "mods/blankStone/files/ui_gfx/inventory/bag_gui_button_drop_orderly.png" then
...
function draw_inventory_multiple_depth_button(pos_x, pos_y, pos_z)
    GuiZSetForNextWidget(gui, pos_z)
    GuiColorSetForNextWidget(gui, bag_ui_red, bag_ui_green, bag_ui_blue, bag_ui_alpha)
    local order_sprite = "mods/blankStone/files/ui_gfx/inventory/bag_gui_button_multi_depth_display.png"
    if not keep_tooltip_open and dropdown_style then
        order_sprite = "mods/blankStone/files/ui_gfx/inventory/bag_gui_button_one_depth_display.png"
    end
    if not dropdown_style then
        order_sprite = "mods/blankStone/files/ui_gfx/inventory/bag_gui_button_right_click_nav.png"
    end
    if GuiImageButton(gui, bags_of_many_new_id(), pos_x, pos_y, "", order_sprite) then
        if dropdown_style then
            ...
            ModSettingSetNextValue("blankStone.keep_tooltip_open", keep_tooltip_open, false)
        end
    end
    ...
    if right_clicked then
        ...
        ModSettingSetNextValue("blankStone.dropdown_style", dropdown_style, false)
    end
end
...
function draw_tooltip(item, pos_x, pos_y, level)
    ...
    if tooltip ~= "" and not is_storageStone(item) and not is_wand(item) then
        GuiBeginAutoBox(gui)
    ...
    elseif is_wand(item) then
        ...
        local spells_per_line = get_mod_setting_number("blankStone.spells_slots_inventory_wrap", 10)
        local wand_capacity = EntityGetWandCapacity(item)
        ...
        -- Background
        local slot_size_x, slot_size_y = GuiGetImageDimensions(gui, "mods/blankStone/files/ui_gfx/inventory/full_inventory_box.png")
...
function draw_wand_spells(wand_capacity, wand_spells, spells_per_line, pos_x, pos_y)
    ...
    for i = 1, wand_capacity do
        ...
        GuiZSetForNextWidget(gui, 3)
        GuiImage(gui, bags_of_many_new_id(), background_pos_x, background_pos_y, "mods/blankStone/files/ui_gfx/inventory/inventory_box_inactive_overlay.png", alpha, 1, 1)
        -- Spell sprite
...
function inventory_slot(gui, pos_x, pos_y, pos_z)
    GuiZSetForNextWidget(gui, pos_z)
    GuiColorSetForNextWidget(gui, bag_ui_red, bag_ui_green, bag_ui_blue, 1)
    GuiImage(gui, bags_of_many_new_id(), pos_x, pos_y, "mods/blankStone/files/ui_gfx/inventory/full_inventory_box.png", bag_ui_alpha, 1, 1)
end
...
function draw_alchemy_button_gui(pos_x, pos_y)
    GuiZSetForNextWidget(gui, 1)
    local clicked, right_clicked = GuiImageButton(gui, bags_of_many_new_id(), pos_x, pos_y, "", "mods/blankStone/files/ui_gfx/potion_mixing/alchemy_button_gui.png")
    local _, _, hovered = GuiGetPreviousWidgetInfo(gui)
...
function potion_alchemy_table(pos_x, pos_y, pos_z)
    ...
    GuiOptionsAddForNextWidget(gui, GUI_OPTION.DrawNoHoverAnimation)
    GuiZSetForNextWidget(gui, 1)
    GuiImageButton(gui, bags_of_many_new_id(), pos_x-12, pos_y, "", "mods/blankStone/files/ui_gfx/potion_mixing/alchemy_move_button_invisible.png")
    local _, _, _, _, _, draw_width, draw_height, draw_x, draw_y = GuiGetPreviousWidgetInfo(gui)
    ...
    GuiZSetForNextWidget(gui, 1)
    GuiImage(gui, bags_of_many_new_id(), pos_x-12, pos_y, "mods/blankStone/files/ui_gfx/potion_mixing/alchemy_move_button.png", 1, 1)
    GuiZSetForNextWidget(gui, pos_z + 4)
    GuiImage(gui, bags_of_many_new_id(), pos_x, pos_y, "mods/blankStone/files/ui_gfx/potion_mixing/alchemy_table.png", 1, 1 ,1)
    GuiZSetForNextWidget(gui, pos_z + 5)
    GuiImage(gui, bags_of_many_new_id(), pos_x -3, pos_y - 3, "mods/blankStone/files/ui_gfx/potion_mixing/alchemy_table_background.png", 1, 1)
end

function potion_alchemy_left_spot(pos_x, pos_y, pos_z)
    GuiZSetForNextWidget(gui, pos_z - 1)
    GuiOptionsAddForNextWidget(gui, GUI_OPTION.NoPositionTween)
    GuiImage(gui, bags_of_many_new_id(), pos_x, pos_y, "mods/blankStone/files/ui_gfx/potion_mixing/potion_left_number.png", 1, 1 ,1)
    local potion_sprite = "mods/blankStone/files/ui_gfx/potion_mixing/potion_empty_spot.png"
    if left_spot_alchemy.item then
        add_potion_color(gui, left_spot_alchemy.item)
        if is_potion(left_spot_alchemy.item) then
            potion_sprite = "mods/blankStone/files/ui_gfx/potion_mixing/potion.png"
        elseif is_powder_stash(left_spot_alchemy.item) then
            potion_sprite = "mods/blankStone/files/ui_gfx/potion_mixing/material_pouch.png"
            pos_x = pos_x - 2
            pos_y = pos_y - 2
        end
    else
...
function potion_alchemy_combined_spot(pos_x, pos_y, pos_z)
    GuiZSetForNextWidget(gui, pos_z - 1)
    GuiOptionsAddForNextWidget(gui, GUI_OPTION.NoPositionTween)
    GuiImage(gui, bags_of_many_new_id(), pos_x, pos_y, "mods/blankStone/files/ui_gfx/potion_mixing/potion_combined_number.png", 1, 1 ,1)
    local potion_sprite = "mods/blankStone/files/ui_gfx/potion_mixing/potion_empty_spot.png"
    if combined_spot_alchemy.item then
...
function potion_alchemy_right_spot(pos_x, pos_y, pos_z)
    GuiZSetForNextWidget(gui, pos_z - 1)
    GuiOptionsAddForNextWidget(gui, GUI_OPTION.NoPositionTween)
    GuiImage(gui, bags_of_many_new_id(), pos_x, pos_y, "mods/blankStone/files/ui_gfx/potion_mixing/potion_right_number.png", 1, 1 ,1)
    local potion_sprite = "mods/blankStone/files/ui_gfx/potion_mixing/potion_empty_spot.png"
    if right_spot_alchemy.item then
...
function potion_alchemy_table_content(potion, pos_x, pos_y, pos_z)
    ...
    if materials and #materials > 0 then
        ...
        for i = 1, #materials do
            ...
            local potion_content_button_sprite = "mods/blankStone/files/ui_gfx/potion_mixing/button_unchecked.png"
            if potion_alchemy_content_buttons[potion] and potion_alchemy_content_buttons[potion][materials[i].id] then
                potion_content_button_sprite = "mods/blankStone/files/ui_gfx/potion_mixing/button_checked.png"
            end
            GuiZSetForNextWidget(gui, 2)
...
function potion_alchemy_delete_chosen(pos_x, pos_y, pos_z)
    local delete_sprite = "mods/blankStone/files/ui_gfx/potion_mixing/thrashcan_disabled.png"
    if is_material_in_alchemy_table_selected() then
        delete_sprite = "mods/blankStone/files/ui_gfx/potion_mixing/thrashcan.png"
    end
    GuiZSetForNextWidget(gui, 3)
    GuiOptionsAddForNextWidget(gui, GUI_OPTION.DrawNoHoverAnimation)
...
function potion_alchemy_transfer(pos_x, pos_y, pos_z)
    local transfer_sprite = "mods/blankStone/files/ui_gfx/potion_mixing/transfer_button.png"
    if not combined_spot_alchemy.item then
        transfer_sprite = "mods/blankStone/files/ui_gfx/potion_mixing/transfer_button_disabled.png"
    end
    GuiZSetForNextWidget(gui, 3)
...
function potion_unselect_all(pos_x, pos_y, pos_z)
    if is_material_in_alchemy_table_selected() then
        local unselect_all_button = "mods/blankStone/files/ui_gfx/potion_mixing/unselect_all_button.png"
        GuiZSetForNextWidget(gui, 3)
...
function setup_inventory_options_buttons(bag, level, pos_x, pos_y, pos_z)
    ...
    if ModSettingGet("blankStone.show_drop_all_inventory_button") then
    ...
    if level == 1 and ModSettingGet("blankStone.show_change_sorting_direction_button") then
        local pos_x_button, pos_y_button = calculate_grid_position(length, size, size, direction, nb_button)
        nb_button = nb_button + 1
        draw_inventory_multiple_depth_button(pos_x + pos_x_button, pos_y + pos_y_button, pos_z)
    end
    if ModSettingGet("blankStone.show_change_sorting_direction_button") then
        local pos_x_button, pos_y_button = calculate_grid_position(length, size, size, direction, nb_button)
        nb_button = nb_button + 1
        draw_inventory_sorting_option(pos_x + pos_x_button, pos_y + pos_y_button, pos_z)
    end
    if sort_by_time_setting and ModSettingGet("blankStone.show_change_sorting_direction_button") then
        local pos_x_button, pos_y_button = calculate_grid_position(length, size, size, direction, nb_button)
        nb_button = nb_button + 1
